...
async def welcome(message: types.Message):
    ...
    if await functions.check_on_start(message.from_user.id):
        await message.answer(f"""Assalomu alaykum! \n\nO'zingizga kerakli bo'limni tanlang""", reply_markup=frond_btn)
        await From.sent_num.set()
    else:
        yubordim = await message.answer("Botimizdan foydalanish uchun kanalimizga azo bo'ling", reply_markup=join_inline)
...
async def check(call: CallbackQuery):
    ...
    if await functions.check_on_start(user_id):
        ...
        await call.message.answer("Xush keldiz", reply_markup=frond_btn)
    else:
